...
app.get("/api/status/:code", async (req, res) => {
  ...
  const { period } = req.query; // Optional period

  res.header("Access-Control-Allow-Origin", "*"); // Allow Dashboard to call
  res.header("Access-Control-Allow-Methods", "GET, OPTIONS");

  if (!code) return res.json({ submitted: false });

  try {
    const periodId = period || CURRENT_PERIOD_ID;
    const { getRectificationByStudent } = require("./utils/adminStore");
    const existing = await getRectificationByStudent(code, periodId);

    if (existing) {
      return res.json({
        submitted: true,
        id: existing.id,
        date: existing.submitted_at,
        pdf: existing.pdf_storage_path ? true : false,
    ...
    return res.status(500).json({ submitted: false, error: err.message });
  }
});
...
app.get("/", (req, res) => {
  // Prevent caching so back button doesn't show portal after logout
  res.set('Cache-Control', 'no-store, no-cache, must-revalidate, private');
  res.set('Pragma', 'no-cache');
  res.set('Expires', '0');

  // ✅ Check if already logged in -> render Dashboard
  if (req.session && req.session.student && req.session.profile) {
    const s = req.session.student;
    ...
    return res.render("index", {
      ...
      studentId: s.codigo,
      semester: fmtPeriod(p.period),
      department: p.specialtyName,
      schedules: enrolled,
      available: [],
      dni: p.dni,
...
app.post("/logout", (req, res) => {
  req.session.destroy(() => {
    // Prevent caching so browser back button doesn't show rectification
    res.set('Cache-Control', 'no-store, no-cache, must-revalidate, private');
    res.set('Pragma', 'no-cache');
    res.set('Expires', '0');
