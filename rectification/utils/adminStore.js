...
async function getRectificationByStudent(studentCode, periodId) {
  let query = supabase
    .from("rectification_requests")
    .select("id, submitted_at, pdf_storage_path, student_code, student_name, period_id, mode, changes") // Select minimal needed
    .eq("student_code", String(studentCode).trim());

  if (periodId) {
    query = query.eq("period_id", String(periodId));
  }

  // Get latest if multiple (though there should be one per period)
  const { data, error } = await query.order("submitted_at", { ascending: false }).limit(1).single();
